          ...
          <div class="flex">
            ...
            <div class="hidden sm:-my-px sm:ml-6 sm:flex sm:space-x-8">
              <!-- <a v-for="item in navigation" :key="item.name" :href="item.href"
                ...
                <NuxtLink   v-for="item in navigation" :key="item.name" :href="item.href" @click="pageNavigated(item)"
                  :class="[item.current ? 'bg-gray-500 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800', 'group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold']"
                 >
                 <!--   -->
                        {{ item.name }}
                </NuxtLink>
...
const pageNavigated = (item) => {
  console.log('pageNavigated',item)
  navigation.value.map((items) => { items.name===item.name?items.current=true:items.current=false})
  // item.current = true
  console.log('pageNavigated',navigation)
}
</script>